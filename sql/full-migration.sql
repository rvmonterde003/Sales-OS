...
INSERT INTO sales_stages (name, stage_order, definition, entry_criteria, exit_criteria, required_fields) VALUES
  ('Discovery', 1,
   ...
   'Lead qualified, first contact made',
   'Clear understanding of needs, demo or audit scheduled',
   ARRAY['service_description', 'source']),
  ('Demonstration/Audit', 2,
   'Presenting the solution or auditing the prospect''s current state to identify gaps.',
   'Discovery completed, stakeholders identified',
   'Prospect has seen the solution or audit findings delivered',
   ARRAY['primary_contact_id']),
  ('Evaluation', 3,
   'Prospect is evaluating the solution — comparing options, assessing fit internally.',
   'Demo/audit completed, prospect actively considering',
   'Prospect confirms interest, requests proposal',
   ARRAY['deal_value']),
  ('Proposal', 4,
   'Formal proposal delivered — pricing, terms, and deliverables presented.',
   'Evaluation positive, budget range confirmed',
  ...
  ('Negotiation', 5,
   ...
   'Terms agreed, moving to contract or verbal',
   ARRAY['forecast_category']),
  ('Contract', 6,
   'Contract drafted and under review — legal and procurement involved.',
   'Terms agreed, contract sent for signature',
   'Contract signed or verbal commitment received',
   ARRAY['contract_value']),
  ('Verbal', 7,
   'Verbal commitment received — awaiting final signed contract.',
   'Decision maker verbally committed',
   'Contract signed (Closed Won) or deal falls through (Loss)',
   ARRAY[]::TEXT[]),
  ('Won', 8,
