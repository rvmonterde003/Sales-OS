import { opportunities, salesStages, getCompanyById, getContactById, formatCurrency, getDealAge, getDaysInStage } from '../data/mockData';
import StatusBadge from '../components/StatusBadge';
import { Link } from 'react-router-dom';
import { Clock, User, SlidersHorizontal, Filter } from 'lucide-react';
            ...
            return (
              <div key={stage.id} className="flex-shrink-0 w-[260px] flex flex-col">
                ...
                <div className="px-3 py-2.5 mb-2">
                  ...
                  {stageTotal > 0 && (
                    <div className="text-[11px] text-gray-400 mt-0.5">{formatCurrency(stageTotal)}</div>
                  )}
                </div>
                ...
                <div className="flex-1 space-y-2 min-h-[100px]">
                  {stageOpps.map(opp => {
                    ...
                    return (
                      <Link
                        ...
                        className="kanban-card block bg-white rounded-lg border border-gray-200 p-3"
                      >
                        ...
                        <div className="flex items-center gap-1.5 flex-wrap mb-2">
                          {opp.forecastCategory && <StatusBadge status={opp.forecastCategory} variant="tag" />}
                        ...
                        <div className="flex items-center justify-between text-[11px] text-gray-400 pt-2 border-t border-gray-100">
                          <span className="flex items-center gap-1">
                            ...
                            {contact ? `${contact.firstName} ${contact.lastName[0]}.` : '—'}
                          </span>
                  ...
                  {stageOpps.length === 0 && (
                    <div className="text-center py-8 text-[11px] text-gray-300">No deals</div>
                  )}
                </div>
              </div>
            );
